...
window.addEventListener("scroll", ()=>{
  ...
  laptop[0].style.transform = `translateX(calc(475vh - ${offsetY}px))`
  phones[1].style.transform = `translateX(calc(600vh - ${offsetY}px))`
  laptop[1].style.transform = `translateX(calc(650vh - ${offsetY}px))`
})

// Get references to all elements with the class 'projectButton'
const buttons = document.querySelectorAll('.projectButton');

// Add a click event listener to each button
buttons.forEach(button => {
  button.addEventListener('click', () => {
    // Get the link URL from the data attribute of the parent projectDetail element
    const linkUrl = button.parentElement.getAttribute('data-link');
...
boxContainer.addEventListener('click',() =>{
  const linkUrl = boxContainer.getAttribute('data-link');
  window.open(linkUrl, '_blank');
});